/*
Package mdbxpool provides a TxnPool type that allows mdbx.Readonly transactions
...
would be absent in a naive use of sync.Pool with mdbx.Txn types.

Naively reusing mdbx.Readonly transactions can cause updates to continually
allocate more pages for the database instead of reusing stale pages.  The
TxnPool type tracks transaction ids to make sure that mdbx.Readonly
transactions are not reused when they are known to be holding stale pages which
could be reclaimed by mdbx.

A general downside of pooling mdbx.Readonly transactions using a sync.Pool in
