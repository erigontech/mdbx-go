...
// path.
func NewEnv(opt *EnvOptions) (env *mdbx.Env, err error) {
	var dir string
	defer func() {
		if err != nil {
			if env != nil {
	...
	dir, err = ioutil.TempDir("", "test-env-")
	if err != nil {
		return nil, err
	}
	env, err = mdbx.NewEnv()
...
func OpenDBI(env *mdbx.Env, name string, flags uint) (mdbx.DBI, error) {
	var dbi mdbx.DBI
...
// commits the transaction, and returns the open DBI handle to the caller.
func OpenRoot(env *mdbx.Env, flags uint) (mdbx.DBI, error) {
	var dbi mdbx.DBI
	err := env.Update(func(txn *mdbx.Txn) (err error) {
...
func Put(env *mdbx.Env, dbi mdbx.DBI, items ItemList) error {
	return env.Update(func(txn *mdbx.Txn) (err error) {
