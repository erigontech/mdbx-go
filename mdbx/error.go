...
const hardwareRecommendations = "Likely because hardware failure. Before creating issue please use tools like https://www.memtest86.com to test RAM and tools like https://www.smartmontools.org to test Disk. To handle hardware risks: use ECC RAM, use RAID of disks, run multiple instances of application (or do backups). If hardware checks passed - check FS settings - 'fsync' and 'flock' must be enabled. "
const coredumpRecommendations = "Otherwise - likely it's application bug - please create issue." // with backtrace or coredump. To create coredump set compile option 'MDBX_FORCE_ASSERTIONS=1' and env variable 'GOTRACEBACK=crash'."
